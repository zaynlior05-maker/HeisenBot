...
def handle_ausfresh_purchase(call):
    ...
    if 1 <= item_id <= len(all_aus_data):
        ...
        product_data = f"Australian Fresh Base Item #{item_id}"
...
def handle_ukfresh_purchase(call):
    ...
    if 1 <= item_id <= len(all_uk_data):
        ...
        product_data = f"UK Fresh Base Item #{item_id}"
    ...
    try:
        ...
        if len(str(bin)) == 6:
            with open("binlist.txt", 'r') as file:
                ...
                for line in lines:
                    if str(bin) in line:
                        inline_keyboard2.add(types.InlineKeyboardButton(line.rstrip(), callback_data='fullz'))
                        exists = True
                
                if exists:
                    bot.send_message(
                        message.chat.id,
                        text="Select from results below:",
                        reply_markup=inline_keyboard2,
                        parse_mode="HTML"
                    )
                else:
                    bot.send_message(
                        message.chat.id,
                        text="BIN not found. Try searching in the specific bases.",
                        parse_mode="HTML"
                    )
        else:
            bot.send_message(
                message.chat.id,
                text="❌ BIN must be exactly 6 digits",
                parse_mode="HTML"
            )
    except ValueError:
        bot.send_message(
            message.chat.id,
            text="❌ Please enter only numbers for BIN search",
            parse_mode="HTML"
        )
    except Exception as e:
...
def callback_query(call):
    if call.data == "store":
        ...
        # Use new callback-based BIN search interface
